...
#ifdef CONFIG_FB_MSM_MDSS_SPECIFIC_PANEL
	rc = panel_driver_request_gpios(ctrl_pdata);
	if (rc)
...
disp_en_gpio_err:
	...
	return rc;
#else
	panel_driver_gpio_free(ctrl_pdata);
specific_panel_err:
	return rc;
#endif /* CONFIG_FB_MSM_MDSS_SPECIFIC_PANEL */
}
...
static int mdss_dsi_post_panel_on(struct mdss_panel_data *pdata)
{
	...
	if (incell && (spec_pdata->panel_type == HYBRID_INCELL)) {
		state = incell->state;
		if ((!panel_driver_is_power_on(state)) &&
		    (!panel_driver_is_ewu(state)))
			panel_driver_reset_touch(pdata, 0);
	}
	panel_driver_reset_touch(pdata, 1);

	if (incell)
		panel_driver_state_change_on(incell);
	panel_driver_post_on(ctrl);
#endif /* CONFIG_FB_MSM_MDSS_SPECIFIC_PANEL */
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	if (incell) {
		panel_driver_power_off_ctrl(incell);
		panel_driver_state_change_off(incell);
	}
#endif /* CONFIG_FB_MSM_MDSS_SPECIFIC_PANEL */
...
#ifdef CONFIG_FB_MSM_MDSS_SPECIFIC_PANEL
	rc = panel_driver_parse_dt(np, ctrl_pdata);
	if (rc) {
		pr_err("%s: failed to parse somc panel features\n", __func__);
		goto error;
	}
#endif /* CONFIG_FB_MSM_MDSS_SPECIFIC_PANEL */
