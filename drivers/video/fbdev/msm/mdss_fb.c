...
#include "panel_driver/panel_debugfs.h"
...
static void mdss_fb_update_early_unblank_completed(struct msm_fb_data_type *mfd,
			bool update)
{
...
static bool mdss_fb_did_early_unblank(struct msm_fb_data_type *mfd)
{
	return mfd->early_unblank_completed;
}

static void mdss_input_event(struct input_handle *handle, unsigned int type,
...
static int mdss_input_connect(struct input_handler *handler,
...
static void mdss_input_disconnect(struct input_handle *handle)
{
...
static int mdss_fb_probe(struct platform_device *pdev)
{
	...
	const char *data;
#ifdef CONFIG_FB_MSM_MDSS_SPECIFIC_PANEL
	struct panel_incell_ctrl *incell = NULL;
	...
	panel_incell_driver_init();
	incell = panel_incell_get_info();
	if (incell) {
		if (mfd->panel_info->cont_splash_enabled)
			incell->state = INCELL_INIT_STATE_LK;
		pr_debug("%s: state:0x%x touch operation:%d\n", __func__,
			(incell->state), (int)(incell->panel_incell_intf_operation));
	}
			     ...
			     int op_enable)
{
	...
	default:
#ifdef CONFIG_FB_MSM_MDSS_SPECIFIC_PANEL
		panel_driver_fb_notifier_call_chain(mfd,
	...
	if (fb_backup->disp_commit.flags & MDP_DISPLAY_COMMIT_OVERLAY) {
		if (mfd->mdp.kickoff_fnc)
			...
			pr_warn("no kickoff function setup for fb%d\n",
					mfd->index);
#ifdef CONFIG_FB_MSM_MDSS_SPECIFIC_PANEL
		panel_driver_fps_data_update(mfd, FPSD);
#endif /* CONFIG_FB_MSM_MDSS_SPECIFIC_PANEL */
